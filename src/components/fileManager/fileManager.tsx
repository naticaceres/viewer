...
class FileManager extends React.Component<ViewerProps, ViewerState> {
  ...
  handleFileRead = (e: any) => {
    ...
    this.parseDxfString(content);
  };

  componentDidMount() {
    if (!this.props.dxfString) {
      return;
    }
    this.parseDxfString(this.props.dxfString);
  }

  parseDxfString(dxfString: string) {
    this.setState({ dxfString });
    const helper = new dxf.Helper(dxfString);
    const parsedFile = {
      svg: helper.toSVG(),
  ...
  render() {
    return (
      !this.props.dxfString && (
        <div className="file-manager" style={{ textAlign: 'left' }}>
